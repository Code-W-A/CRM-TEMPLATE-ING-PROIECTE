...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await safeAddLog(
      "Notificare API",
      `Încercare trimitere notificări pentru proiect ${workOrderId}`,
      "Informație",
      "Email",
    )
    ...
    console.log(`- EMAIL_USER: ${process.env.EMAIL_USER || "CRM@nrg-acces.ro"}`)
    ...
    const clientWorkOrderInfo = isPostponed
      ...
      <div style="padding: 10px 12px; border-left: 4px solid #8b5cf6; background:#f5f3ff; border-radius:4px; margin-bottom: 12px;">
        <div style="font-weight:600; color:#53389e; margin-bottom:6px;">Stare proiect: Amânată</div>
      ...
      <ul style="list-style-type: none; padding-left: 0;">
        ...
        ${details?.workType ? `<li><strong>Tip proiect:</strong> ${details.workType}</li>` : ''}
      </ul>
    `
      : `
      <ul style="list-style-type: none; padding-left: 0;">
    ...
    // Construim URL-ul corect pentru proiect
    const workOrderUrl = `${appBaseUrl}/dashboard/lucrari/${safeWorkOrderId}`
    console.log(`[WORK-ORDER-API] [${requestId}] URL proiect generat: ${workOrderUrl}`)
          ...
          try {
            ...
            logInfo(
              "Sending email to technician",
              {
                ...
                subject: `proiect nouă: ${client?.name}`,
              },
              { category: "email", context: logContext },
            )
            ...
            const htmlContent = `
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <h2 style="color: #0f56b3;">proiect nouă asignată</h2>
                <p>Salut ${tech.name},</p>
                <p>Ai fost asignat la o nouă proiect:</p>
                
                <!-- Buton de acces proiect -->
            ...
            const mailOptions = {
              ...
              to: tech.email,
              subject: `proiect nouă: ${client?.name}`,
              text: `Salut ${tech.name}, ai fost asignat la o nouă proiect pentru clientul ${client?.name || "N/A"}. Accesează proiecta la: ${workOrderUrl}`,
              html: htmlContent,
              attachments: [
                {
                  filename: "logo.png",
      ...
      try {
        ...
        logInfo(
          "Sending email to client",
          {
            ...
            subject: isPostponed ? `Anunț amânare proiect: ${details?.location || "Locație nedefinită"}` : `Confirmare proiect: ${client?.name}`,
          },
          { category: "email", context: logContext },
        )
        ...
        const htmlContent = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <h2 style="color: #0f56b3;">${isPostponed ? "proiect amânată" : "Confirmare proiect"}</h2>
            <p>Stimate ${client.contactPerson || client.name},</p>
            <p>${isPostponed
              ? "Vă informăm că proiecta a fost amânată. Mai jos regăsiți detaliile relevante:"
        ...
        const mailOptions = {
          ...
          to: uniqueRecipients,
          subject: isPostponed
            ? `Anunț amânare proiect: ${details?.location || "Locație nedefinită"}`
            : `Confirmare intervenție: ${details?.location || "Locație nedefinită"}`,
          text: isPostponed
            ? `Stimate ${client.contactPerson || client.name}, vă informăm că proiecta a fost amânată.${details?.postponeReason ? ` Motiv: ${details.postponeReason}.` : ''}`
    ...
    await safeAddLog(
      "Notificare API",
      `Rezultat trimitere notificări pentru proiecta ${workOrderId}: ${technicianEmails.length} tehnicieni, client: ${clientEmailResult?.success ? "Succes" : "Eșec"}`,
      "Informație",
      "Email",
    )
