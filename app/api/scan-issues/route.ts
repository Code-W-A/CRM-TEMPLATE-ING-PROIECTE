...
export async function POST(req: NextRequest) {
  try {
    ...
    await adminDb.collection("logs").add({
      ...
      detalii: `proiect: ${lucrareId}; expected: ${requestData.context.expectedEquipmentCode || '-'}; raw: ${latestDetectedCodeRaw || '-'}; încercări: ${requestData.lastScan.failedScanAttempts}`,
      tip: "Eroare",
      categorie: "Scanare",
      extra: {
        requestId: docRef.id,
