...
export default function Lucrari() {
      ...
      const dataToSubmit = {
        ...
        ...(isReassignment && originalWorkOrderId
          ? {
              ...
              mesajReatribuire: `Re-intervenție pentru: ${formData.originalWorkOrderInfo || `proiect ${originalWorkOrderId}`}`,
            }
          : {}),
      }
    ...
    try {
      ...
      const newLucrare = {
        ...
        ...(isReassignment && originalWorkOrderId
          ? {
              lucrareOriginala: originalWorkOrderId,
              mesajReatribuire: `Re-intervenție pentru: ${formData.originalWorkOrderInfo || `proiectul ${originalWorkOrderId}`}`,
  ...
  const handleViewDetails = (lucrare) => {
    ...
    if (isTechnician && (isCompletedWithReportNotPickedUp(lucrare) || isPostponedNotPickedUp(lucrare))) {
      toast({
        title: "Acces restricționat",
        description: "Proiectul este în așteptare de preluare de către dispecer.",
        variant: "default",
          ...
          >
            <DialogTrigger asChild>
              <Button className="bg-blue-600 hover:bg-blue-700">
                <Plus className="mr-2 h-4 w-4" /> <span className="hidden sm:inline">Adaugă</span> Proiect
              </Button>
            </DialogTrigger>
              ...
              {paginatedCardsData.length === 0 && filteredData.length === 0 && (
                <div className="col-span-full text-center py-10">
                  {userData?.role === "tehnician" ? (
                    ...
                    <p className="text-muted-foreground">Nu există proiecte care să corespundă criteriilor de căutare.</p>
                  )}
                </div>
              )}
            </div>
